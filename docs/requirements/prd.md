...
- **体系化学习**：全书共 Ch0 + 7 章 + 附录 A，50 篇文章，从核心概念到高级话题逐步深入
...
| 指标 | 目标值 | 衡量方式 |
|------|--------|---------|
| 内容完整度 | 50 篇文章全部有完整内容（第 6、7 章部分文章篇幅较短，2-7KB） | 文章数量统计 |
| 代码示例 | 100+ 个可运行的代码/配置示例 | 代码块统计 |
| 图表数量 | 50+ 张 Mermaid/架构图 | 图表数量统计 |
| 内容质量 | 每篇文章 > 200 行有效内容（第7章 ≥ 300 行） | 文件行数统计 |
| 总有效行数 | ≥ 10,000 行（50 篇 × 200 行基线，第7章文章 ≥ 300 行） | 文件行数统计 |
...
- [ ] 所有 50 篇文章完成编写（全部有内容，第 6、7 章部分文章篇幅较短）
...
- [ ] 代码示例总数：≥ 100 个
- [ ] Mermaid/架构图总数：≥ 50 张
- [ ] 全书写稿总有效行数：≥ 50 × 200 = 10,000 行（第7章文章 ≥ 300 行）
- [ ] ADR 示例：≥ 3 个
...
将 45 个用户故事（13 角色）映射到 50 篇文章（含附录 A 术语表），确保全量覆盖。★标记新增文章。

> **注意**：`src/01-introduction/failure-cases.md`（427 行，已完成）原为游离页面，已于 2026-06-03 正式注册为 Ch1 第 6 篇文章（Article 1.6），现计入 50 篇文章统计。
...
> **版本**: v2.5 | 最后更新：2026-06-03 | 第五次更新：文章总数修正为 50 篇（全部有内容），全篇语言优化（reader-centric rewrite）
