...
- **知识体系**：提供完整的 Harness Engineering 知识体系（Ch0 + 7 章，46 篇/页）
...
| 角色 | 标识 | 关注点 | 推荐阅读路径 | 预计占比 |
|------|------|--------|------------|---------|
| 入门开发者 | BEGINNER | 快速上手、核心概念 | Ch0 → Ch1 → Ch2 → Ch3 | 25% |
| 效率开发者 | POWER | 工作流优化、高级技巧 | Ch0 → Ch2 → Ch4 → Ch6 | 20% |
| 技术负责人 | LEAD | 团队推广、工程化体系 | Ch0 → Ch1 → Ch2 → Ch4 → Ch7 | 15% |
| Skill 作者 | SKILL | 开发模板、最佳实践 | Ch0 → Ch5 → Ch6 | 5% |
| 工程经理 | MANAGER | 投资回报、工具选型 | Ch0 → Ch1 → Ch7 | 5% |

### 5.2 扩展读者角色（团队角色Review补充）

| 角色 | 标识 | 关注点 | 推荐阅读路径（★标注新增文章） | 预计占比 |
|------|------|--------|----------------------------|---------|
| **需求分析师/产品经理** | ANALYST | 需求完整性、价值覆盖 | Ch0 → Ch1(★理论框架/★工具对比) → Ch7(★全流程自动化) → Ch4(★Teams协作) | 5% |
| **系统架构师/技术顾问** | SYSA | 技术选型决策、集成架构、安全合规 | Ch1(★工具对比/★理论框架) → Ch6(★安全总览/★可观测性/★CLAUDE.md) → Ch2(★约束系统/★验证护栏) → Ch4(★Agent派生/★Teams协作) → Ch5(★Skill-MCP桥接) → Ch7(★混合架构) | 10% |
| **后端开发者/API工程师** | BACKEND | MCP服务端开发、API契约、数据库安全 | Ch2(★上下文工程/★约束系统) → Ch3(★多环境部署) → Ch6(★沙箱Hook/★Token预算/★可观测性) → Ch4(★Agent派生) → Ch5(★Skill-MCP桥接) → Ch7(★全流程自动化) | 15% |
| **前端开发者/UI工程师** | FRONTEND | 前端场景Agent编排、组件↔Skill类比 | Ch2(★上下文工程/★验证护栏) → Ch5(★Skill插件化/★MCP桥接) → Ch4(★Agent派生/★Teams协作) → Ch6(★可观测性) | 10% |
| **安全工程师/安全架构师** | SECURITY | 安全基线、威胁建模、合规 | Ch2(★约束系统/★验证护栏) → Ch3(★多环境部署/国产模型安全) → Ch6(★安全总览/★沙箱Hook/★CLAUDE.md) → Ch4(★Agent派生安全) → Ch7(★安全审计) | 10% |
| **安全研究人员/红队成员** | REDTEAM | 攻击面评估、Agent安全测试 | Ch2(★上下文工程安全/★约束系统绕过) → Ch6(★安全总览/★沙箱Hook) → Ch5(★Skill插件化安全) → Ch7(★安全审计/★Skill市场) | 10% |
| **文档UX专家** | UX | 文档可读性、Mermaid规范、无障碍 | 全书按需审校（特别注意★工具对比雷达图/★监控仪表板/★架构总览图的可视化质量） | N/A(顾问) |
| **技术审校/QA编辑** | QA | 质量门禁、代码可运行性、一致性 | 全量审校（特别关注★新增27篇文章的验证标准完备性） | N/A(内部) |
...
- [ ] 代码示例总数：≥ 100 个
- [ ] Mermaid/架构图总数：≥ 50 张
- [ ] 全书写稿总有效行数：≥ 46 × 200 = 9,200 行（第7章文章 ≥ 300 行）
- [ ] ADR 示例：≥ 3 个
...
将 45 个用户故事（13 角色）映射到 46 篇文章，确保全量覆盖。★标记新增文章。
...
| ANALYST | US-A-03 读者旅程 | Ch0 (3篇), Ch1 导览 | ✅ 覆盖 |
...
> **版本**: v2.2 | 最后更新：2026-06-01 | 第二次团队角色Review后更新（46篇对齐）
