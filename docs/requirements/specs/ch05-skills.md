...
本章面向需要创建自定义 Skill 的读者——无论你是团队技术负责人想封装最佳实践，还是 Skill 作者想发布到市场。从格式规范到发布流程，4 个可复用模板（调查研究/架构设计/代码审查/敏捷活动）+ 最佳实践和反模式清单。学完即可独立创建并发布一个生产级 Skill。

**章节核心主题**：从会用 Skill 到会写 Skill——模板驱动的最佳实践指南。
...
| 类型 | 推荐 | 理由 |
|------|------|------|
| **思维框架** | 实践论（迭代改进模板）、批评与自我批评（反模式审查） | Skill开发是全书最具实践性的章节，模板质量和反模式总结决定读者产出质量 |
| **人物视角** | Karpathy（系统构建方法论）、Feynman（深层理解 vs 表面模仿） | 同上 |
...
#### 核心概念
- **description 的精确设计**：太宽容易误触发，太窄难以匹配。在 description 中包含触发词 + 场景 + 排除词。
...
**创作辅助**:
- 思维框架：实践论（跟随创建过程走一遍）
- 人物视角：Karpathy（系统构建方法论）
- 理由：创建Skill是本章读者最核心的需求
...
**创作辅助**:
- 思维框架：批评与自我批评（反模式来自真实踩坑经验）
- 人物视角：Feynman（区分真理解和假理解）
- 理由：最佳实践和反模式是真正的价值所在
...
**创作辅助**:
- 思维框架：没有调查没有发言权（MCP桥接需实际测试）
- 人物视角：Karpathy（集成工程思维）
- 理由：Skill-MCP桥接是高级集成场景
...
**创作辅助**:
- 思维框架：群众路线（市场机制需创作者角度）
- 人物视角：Naval（平台与创作者经济）
- 理由：Skill市场是生态系统的关键组成部分
