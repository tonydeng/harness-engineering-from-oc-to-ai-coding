...
| 类型 | 推荐 | 理由 |
|------|------|------|
| **思维框架** | 实践论（每一步作者亲自走一遍）、星星之火（最小可行配置优先） | 环境搭建是所有读者的必经之路，步骤的可复现性决定全书成败 |
| **人物视角** | Feynman（验证每个断言）、Karpathy（工程严谨性） | 同上 |
...
**创作辅助**:
- 思维框架：星星之火（从最简配置开始）
- 人物视角：Karpathy（配置即代码的严谨性）
- 理由：opencode.json是全书最重要的配置文件
...
**创作辅助**:
- 思维框架：实践论（OMO集成需端到端验证）
- 人物视角：Karpathy（系统集成思维）
- 理由：OMO集成涉及多个层面的配置
