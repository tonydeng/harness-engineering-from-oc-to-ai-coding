...
| **US-POWER-05** | 成本优化 | `06-advanced/performance-tuning.md`（三层成本优化模型 + 类别路由 + 自动降级链 + Token 预算 + 上下文压缩 + 工具输出保护）、`06-advanced/token-budget.md`（Token 预算分配 + 超限处理 + 降级策略） | **部分** |
