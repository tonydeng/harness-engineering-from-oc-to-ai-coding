...
| 优先级 | 项数 | 说明 |
|--------|------|------|
| **P0 🔴** | 1 | D3 角色覆盖评分低（平均 3.22/6，64%仅 3 分）需要优先解决 |
| **P1 🟡** | 1 | 7 角色评分栏待填充，需补充评分数据 |
| **P2 🟢** | 1 | 章节级别评分汇总完后可作为考核基线 |

### P0 修复项

| # | 问题 | 目标 | 操作 | 验证 |
|---|------|------|------|------|
| S-P0-1 | **D3 角色覆盖平均 3.22/6** | `src/**/README.md` 及文章首页 | 补充 `> **适合读者**:` 角色标签，提升到≥4角色/篇 | D3 均值≥4.0/6 |

### P1 修复项

| # | 问题 | 目标 | 操作 | 验证 |
|---|------|------|------|------|
| S-P1-1 | **7 角色评分栏待填充** | `scored-articles-2026-06-29.md` 评分表 | 补充各角色维度的逐篇评分数据 | 评分表完整 |

### 检查清单

- [x] S-P0-1: D3 角色覆盖均值已提升（12章追加 `> **适合读者**:` 标签 ✅）
- [ ] S-P1-1: 评分栏填充完整
- [ ] ✅ 最终验证：HEDQ 重评 D3 分数提升
