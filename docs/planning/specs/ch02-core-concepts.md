...
本章是全书的知识基础，系统讲解OpenCode的6大核心概念：Agent（执行主体）、Skill（专业技能包）、Command（快捷指令）、Plugin（扩展能力）、Team（多Agent协作）、MCP（外部协议）。每个概念按"是什么→怎么用→最佳实践"三步展开，用你熟悉的操作系统做类比来帮你建立直觉。读懂这一章，后面各章节你才能跟得上。

**章节核心主题**：搞懂6大核心概念各自是做什么的、它们之间怎么配合——这是 Harness Engineering 的"零件箱"。
...
| 类型 | 推荐 | 理由 |
|------|------|------|
| **思维框架** | 实践论（构建可运行示例验证）、批评与自我批评（审查概念准确性） | 核心概念是全书知识基础，类比和解释的准确性决定读者是否能跟上后续内容 |
| **人物视角** | Karpathy（构建即理解）、Feynman（反自我欺骗——真的理解了吗？） | 同上 |
...
**创作辅助**:
- 思维框架：批评与自我批评（审查Skill边界描述）
- 人物视角：Feynman（深层理解 vs 表面用法）
- 理由：Skill是最具差异化的概念
...
**创作辅助**:
- 思维框架：实践论（Command需可执行验证）
- 人物视角：Karpathy（系统思维）
- 理由：Command是日常使用频率最高的功能
...
- 增加《马书》第4章 Agent Loop 状态机视角的对比
- 补充 OpenCode Agentic Loop 与《马书》第4章框架的异同分析

**Article 2.3（工作流模式）**：
- 增加《马书》第6章 6 种工作流模式引用
- 补充《马书》第6章工作流与 OpenCode 工作流的映射关系
...
| 模式名称 | 描述 | 价值 |
|---------|------|------|
| **显式状态重建** | Agent Loop 状态管理，避免"忘记重置字段"bug | 防止状态不一致 |
| **扣留-释放机制** | 可恢复错误的延迟暴露，防止过早终止 | 提高容错性 |
| **三层上下文模型** | 系统/项目/会话三层上下文分离 | 清晰的职责边界 |
...
| 模式名称 | 描述 | 价值 |
|---------|------|------|
| **分段记忆化** | 大型提示词的缓存优化架构 | 提高缓存命中率 |
| **缓存边界分治** | 多用户共享提示词前缀的全局缓存策略 | 降低成本 |

**权限模型**：
- 4 层安全架构：权限分层 → 技能隔离 → 沙箱隔离 → 注入防御
- Permission Rule 引擎：allow/deny/ask + glob 匹配 + 优先级
- 敏感文件保护：默认 deny .env, node_modules

**配置合并策略**：
- 四级优先级层叠：全局 → 项目 → 环境变量 → CLI flag
- 条件规则按需加载：上下文预算优化
- 命名空间化去重：多来源配置合并
...
- **阅读时间**：15 min
- **学习目标**：
  - 理解验证护栏在 AI 编程工作流中的角色
  - 掌握质量门禁的设计模式
  - 理解 YOLO 分类和自动验证机制
- **前置知识**：Article 2.5（约束系统）
- **源材料映射**：《马书》第17章，HE实践 03 §实践三
...
| 模式名称 | 描述 | 价值 |
|---------|------|------|
| **分层短路过滤** | AI 分类器的成本优化策略 | 降低分类成本 |
| **危险 Bash 模式检测** | 保护 AI 分类器的安全边界 | 防止命令注入 |
...
**质量门禁**：
- preCommit：lint、test、typeCheck
- prePush：coverage、security audit
- manual：手动触发
...
| 阶段 | 本阶段活动 | 交付物 | 负责人 |
|------|-----------|--------|--------|
| **头脑风暴** | 确定 6 个核心概念的读者理解难点、收集《马书》引用材料、识别安全/架构增强点 | 概念难点清单、增强需求 | 架构顾问 |
| **计划** | 排序写作依赖（2.1→2.2→2.3→2.4→2.5→2.6）、分配角色、确定《马书》引用范围 | 写作计划、引用清单 | 敏捷教练 |
