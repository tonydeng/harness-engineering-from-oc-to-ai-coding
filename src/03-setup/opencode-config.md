...
flowchart TB
    ...
    subgraph Layer4["第 4 层：项目配置"]
        P1["./.opencode/config.json<br/>项目推荐路径"]
    end
...
| `options.setCacheKey` | boolean | 启用 Prompt Cache 密钥（Anthropic 专用，默认 false） |
...
MCP（Model Context Protocol）服务器扩展 Agent 的能力边界。**注意**：`command` 字段为数组类型，同时包含命令和参数：
...
| `repo_clone` | ✅ glob 匹配 | 克隆仓库 |
| `repo_overview` | ✅ glob 匹配 | 仓库概览 |
| `lsp` | ✅ glob 匹配 | LSP 工具 |
...
OpenCode 不提供内置的 Token 预算或费用上限配置。成本管控的核心思路是通过**模型选择**、**Provider 路由**和**上下文优化**等策略，在实际使用中控制 API 调用成本。

> 此成本对比为写书时（2026年6月）所查询数据，请以当前实际定价为准。

### 策略一：按任务类型选择模型

在类别路由配置中，为不同复杂度的任务分配不同能力的模型，避免"杀鸡用牛刀"：
...
```json
{
  "categories": {
    "quick": {
      "model": "fast-model",
      "description": "简单任务使用轻量模型，大幅降低成本"
    },
    "writing": {
      "model": "balanced-model",
      "description": "文档写作使用平衡模型"
    },
    "ultrabrain": {
      "model": "best-capability-model",
      "description": "复杂逻辑使用最强模型，但使用频率最低"
    }
  }
}
```

### 策略二：Provider 路由到低成本供应商

OpenCode 支持 75+ 模型供应商，可通过类别路由将不同任务导向成本更低的 Provider：
  ...
  "categories": {
    "quick": {
      "fallbackChain": [
        ...
        { "providers": ["github-copilot"], "model": "fast-model" },
        { "providers": ["anthropic"], "model": "fast-model" }
      ]
    },
    "ultrabrain": {
      "fallbackChain": [
        { "providers": ["anthropic"], "model": "best-capability-model" },
        { "providers": ["google"], "model": "best-capability-model" }
      ]
    }
  }
}
...
**推荐策略**：
- **简单任务** → DeepSeek 或 GPT-4o-mini（成本最低）
- **常规任务** → Claude Sonnet 或 Gemini（性价比高）
- **复杂任务** → Claude Opus 或 GPT-4o（能力优先）
- **代码审查** → DeepSeek Reasoner（推理过程可解释）

### 策略三：配置降级链

当首选模型不可用时，自动降级到备用模型，避免因模型不可用而中断工作流：
...
OpenCode 的 `compaction` 配置可以在上下文接近限制时自动压缩历史对话，减少 Token 消耗：
...
|------|------|--------|
| `enabled` | 是否启用压缩 | `true` |
| `threshold` | 触发压缩的上下文使用率（0-1） | `0.75–0.85` |
| `strategy` | 压缩策略：`summarize`（摘要）或 `drop`（丢弃） | `summarize` |

### 策略五：定期审视 API 用量

建议采取以下措施持续管控成本：

1. **监控 Token 消耗**：定期查看 Provider 控制台的用量统计
2. **清理长会话**：长时间运行的会话会积累大量上下文，定期重启新会话
3. **使用国产模型**：国产 Provider（DeepSeek、Qwen 等）价格通常为国际模型的 1/10–1/30
4. **利用缓存**：DeepSeek 等 Provider 支持上下文缓存，可节省 75–90% 的输入成本
