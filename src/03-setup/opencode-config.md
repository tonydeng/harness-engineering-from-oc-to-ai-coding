...
> **内置 Skill 辅助**：OpenCode 内置了 `customize-opencode` Skill，当你对配置字段的格式不确定时，可通过 `skill(name="customize-opencode")` 获取配置格式的权威参考，涵盖 `opencode.json` 的全部字段类型和约束。完整内置 Skill 列表见 [附录 B 内置 Skill 参考](../appendix-b/opencode/capabilities.md#内置-skill-参考)。
