...
这篇文章逐一拆解配置文件的每个关键段：Agent 定义、Skills 注册、MCP 服务器集成、权限规则引擎、类别路由系统（oh-my-openagent 扩展功能）。类别路由（Category Routing）决定了 Agent 如何根据任务类型自动分派到合适的模型，是整个工作流引擎的调度核心。读完本文，你将能够手写或评审一份工程级的 opencode.json 配置。
