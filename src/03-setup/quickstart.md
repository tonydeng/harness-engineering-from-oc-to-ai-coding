...
flowchart TB
    subgraph macOS
        A1[Homebrew] --> B1["brew install anomalyco/tap/opencode"]
...
使用 OpenCode 官方 tap 可以获得最新版本。Homebrew 官方 formula 更新频率较低。
...
3. 输入你的 Anthropic API Key（以 `sk-ant-` 开头）
...
在项目级配置文件 `.opencode/config.json` 中引用：
...
| `/diff` | 查看文件差异 | 检查 OpenCode 做了哪些修改 |
| `/share` | 分享对话 | 生成可分享的对话链接 |
| `/plan` | 切换到 Plan 模式 | 进入只读分析模式 |
...
### 排除敏感目录：.opencodeignore

创建 `.opencodeignore` 文件，排除敏感目录和文件：
...
- → [oh-my-openagent 集成](oh-my-openagent.md) — 扩展 OpenCode 的能力
