...
个人开发者在笔记本上跑 OpenCode 和团队在生产环境中运行 OpenCode 是两回事。不同环境对模型选择、权限级别、Token 预算、安全策略有完全不同的需求。本地开发可能用低成本模型加高权限，CI/CD 需要低权限加快速模型，生产环境则要求严格权限控制和高 Token 预算。读完本文，你将能够为开发、CI/CD、生产三套环境创建 Profile 继承模板，并实现团队级配置治理与 Secret Store 集成。
