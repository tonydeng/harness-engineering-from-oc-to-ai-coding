...
| 项目 | 描述 | GitHub |
|-----|------|--------|
| **claude-code** | 核心 CLI 工具 | anthropics/claude-code（133K Stars）|
...
- **Agent SDK (TypeScript)**：`npm install @anthropic-ai/claude-agent-sdk` — `query()` async 生成器
- **Agent SDK (Python)**：`pip install claude-agent-sdk` — `query()` async 生成器
- **CLI 单行**：`claude -p "prompt"` — 适合简单脚本和 CI/CD
- **Managed Agents (REST API)**：Anthropic 托管的代理服务，适合生产级无沙箱运维
...
| GitHub Stars | 133K（2026-06） | 176K（2026-06）|
