...
```javascript:appendix-e/mimocode/workflow-api.js
// 传统方式（自然语言）
...
```javascript:appendix-e/mimocode/migration-workflow.js
// 项目迁移工作流
...
```javascript:appendix-e/mimocode/agent-example.js
await agent({
  prompt: "分析项目结构和依赖",
...
```javascript:appendix-e/mimocode/parallel-example.js
...
```javascript:appendix-e/mimocode/pipeline-example.js
await pipeline([
...
```javascript:appendix-e/mimocode/workflow-example.js
await workflow("./scripts/deploy.js", {
  timeout: 60000,
  retry: 1
});
...
```javascript:appendix-e/mimocode/foreach-example.js
...
```javascript:appendix-e/mimocode/condition-example.js
await condition(
...
```javascript:appendix-e/mimocode/retry-example.js
await retry(
  () => agent({ prompt: "运行测试" }),
...
```javascript:appendix-e/mimocode/timeout-example.js
await timeout(
  () => agent({ prompt: "复杂分析任务" }),
  30000
);
...
```javascript:appendix-e/mimocode/complete-workflow.js
export default async function migrateProject() {
  // 1. 分析现有代码（串行执行）
  const analysis = await agent({
    prompt: "分析项目结构和依赖",
...
```javascript:appendix-e/mimocode/dream-hooks.js
// 注册 Dream 钩子
await dream({
  hooks: {
    preDream: async () => {
...
```javascript:appendix-e/mimocode/skill-hooks.js
...
```javascript:appendix-e/mimocode/composite-skill.js
...
```javascript:appendix-e/mimocode/maxmode-config.js
const maxModeConfig = {
  parallelism: 8,
  temperature_range: {
    min: 0.5,
...
```javascript:appendix-e/mimocode/config-validate.js
