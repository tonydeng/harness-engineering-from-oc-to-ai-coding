...
flowchart TB
    ...
    subgraph Agent层["Agent 层"]
        ...
        Sub["Subagent<br/>@general / @explore"]
        Hidden["Hidden Agent<br/>compaction / title"]
    end
    
    subgraph Tool层["Tool 层"]
        FileTools[文件工具<br/>Read/Write/Edit/Delete]
        BashTools[命令工具<br/>RunCommand]
        NetTools[网络工具<br/>WebSearch/WebFetch]
...
> 1. [Read] 查看现有项目结构
> 2. [Read] 分析现有认证逻辑
> 3. [Write] 创建 src/auth/login.ts
> 4. [Write] 创建 src/auth/login.test.ts
> 5. [RunCommand] npm test
> 6. [Edit] 修复测试失败
```
...
> @scout Agent：
> 1. [Glob] 扫描项目根目录结构
> 2. [Read] 读取 package.json 和 tsconfig.json
> 3. [Read] 读取 README.md 了解项目目标
> 4. [输出] 项目全景分析报告
```
...
OMO 提供了 13+ 个专业 Agent，每个 Agent 负责特定类型的工作：
...
《Build AI Agent that Actually Work》（简称"马书"）提出了 Agent Loop 的状态机视角，与 OpenCode 的 Agentic Loop 有异曲同工之妙。
...
| **Executing** | 工具执行 | Read/Write/RunCommand 等 |
