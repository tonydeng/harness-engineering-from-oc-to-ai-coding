...
YOLO 智能决策核心是验证护栏的决策引擎。名字借用了 You Only Look Once，但它不是机器学习分类器——实际是**多层规则匹配引擎**，根据操作内容判定风险等级：高风险（阻止执行）、中风险（确认后执行）、低风险（自动执行）。自动验证机制涵盖 LSP 验证链（语法→类型→lint→语义）、测试自动执行和架构符合性检查。本章还分析攻击者如何利用 YOLO 误判绕过门禁、伪造通行报文等风险及防御策略。学完本节，你应能设计并配置适配项目质量要求的验证体系。
