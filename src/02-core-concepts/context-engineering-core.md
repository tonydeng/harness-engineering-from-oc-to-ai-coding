...
**压缩比 vs 保真度的权衡**：更高的压缩比意味着更多的信息损失。OpenCode 默认在压缩比 3:1 时保持 85% 以上的关键信息保真度。
...
| 策略 | 解决的问题 | 核心机制 | 效果 |
|------|------------|----------|------|
| **缓存** | 消除重复传输 | 一次发送，多次复用 | 节省 30-50% Token |
| **压缩** | 精简必要内容 | 选择性保留，丢弃冗余 | 延长 2-3 倍会话寿命 |
...
- → [§6.4 上下文压缩技术](../06-advanced/context-compression.md)：压缩机制的深入原理与调优
- → [§6.5 Token 预算策略](../06-advanced/token-budget.md)：预算分配的详细策略
- → [§6.6 提示词缓存机制](../06-advanced/prompt-caching.md)：缓存机制的完整实现
