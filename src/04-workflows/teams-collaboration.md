...
1. **初始化 Team 配置**：加载 JSON 配置文件中定义的 Team 名称、成员角色、Skill 依赖和权限策略
2. **创建 Team Lead**：实例化协调者 Agent，加载顶层规划 Skill（如 `overall-planning`、`dispatching-parallel-agents`），设置消息路由权限
3. **创建 Worker 池**：根据 `minWorkers` 批量实例化 Worker Agent，每个 Worker 拥有独立的 Prompt 上下文和 Skill 组合
4. **初始化 Inbox 目录**：在 `~/.omo/runtime/{teamRunId}/inboxes/` 下为每个成员创建收件目录，供消息持久化和异步投递使用
...
1. **停止接收新任务**：Team Lead 停止分配新任务，拒绝新的 `team_send_message` 请求
2. **等待任务完成**：等待所有已在执行的任务在 `taskCompletionTimeout`（默认 300 秒）内自然完成；超时未完成的由 `forceKillTimeout`（默认 60 秒）强制终止
3. **清理资源**：回收 Agent 会话、关闭 Inbox 文件句柄、清空 `~/.omo/runtime/{teamRunId}/` 下的运行时目录
4. **销毁 Team**：调用 `team_delete` 删除 Team 记录，释放所有会话和 Inbox 文件
