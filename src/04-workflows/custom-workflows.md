...
→ Hyperplan 是 [多 Agent 协作](multi-agent-collab.md) 中竞争模式（Adversarial）的形式化扩展。本文将详细介绍 Hyperplan 的实现细节。
...
```json:.opencode/skills/hyperplan/SKILL.md
{
  "skill": "hyperplan",
  "version": "1.0.0",
  "team_create": {
...
```json:.opencode/skills/security-research/SKILL.md
{
...
```json:.opencode/workflows/user-auth-implementation.json
{
  "workflow": {
...
```json:.opencode/workflows/pr-review-pipeline.json
{
  "workflow": {
...
```json:.opencode/workflows/doc-generation.json
{
  "workflow": {
    "name": "doc-generation",
...
> ¹ **关于嵌套团队的说明**：12 个 `team_*` 工具在 API 层面不支持嵌套团队（即不能在一个团队内部再通过 `team_create` 创建子团队），这是为了防止资源无限扩张。但更高层级的分层协调可以通过 Teams 多进程协作模式实现——父 Team 通过消息传递机制协调子 Team，形成 Level 1（项目级）→ Level 2（领域级）→ Level 3（执行级）的分层架构。详见 [Teams 并行 Agent 协作](teams-collaboration.md) 的"分层 Team 架构"一节。
