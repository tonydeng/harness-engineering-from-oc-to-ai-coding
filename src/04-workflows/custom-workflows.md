...
→ Hyperplan 是 [多 Agent 协作](multi-agent-collab.md) 中竞争模式（Adversarial）的形式化扩展：将 3 Agent 的基础辩论升级为 5 个专业评审者的交叉批评机制，并增加了 `consensus_threshold`（共识阈值）和 `veto_power`（一票否决）等高级配置。
...
> ¹ **关于嵌套团队的说明**：12 个 `team_*` 工具在 API 层面不支持嵌套团队（即不能在一个团队内部再通过 `team_create` 创建子团队），这是为了防止资源无限扩张。但更高层级的分层协调可以通过 Teams 多进程协作模式实现——父 Team 通过消息传递机制协调子 Team，形成 Level 1（项目级）→ Level 2（领域级）→ Level 3（执行级）的分层架构。详见 [Teams 多进程协作](teams-collaboration.md) 的"分层 Team 架构"一节。
