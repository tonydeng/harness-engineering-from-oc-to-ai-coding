...
> 注：完成度为 Agent 的自我评估结果，建议配合 `tests_pass`、`coverage>N` 等客观验证条件使用，以获得更可靠的停止判断。
...
| `on_max_turns` | 达到最大迭代时的行为：`report`（汇报进度）、`continue`（继续执行）或 `abort`（终止流程） |
| `exploration.max_files` | 探索阶段最多读取的文件数 |
| `exploration.priority_patterns` | 优先探索的文件模式 |
| `verification.lsp_enabled` | 是否启用 LSP 类型检查 |

> 需要安装对应语言的 LSP server（TypeScript 需 `typescript-language-server`，Rust 需 `rust-analyzer`）。
