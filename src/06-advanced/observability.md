...
指标按 `agent`、`model`、`tool` 等标签区分维度。PromQL 查询示例：
...
Loki 是 Grafana 生态的日志聚合系统，与 Prometheus 共享标签体系，可以在同一个 Grafana 面板中关联指标和日志。
...
Filebeat -> Elasticsearch -> Kibana 的组合更适合需要全文搜索和复杂聚合的场景。

```json:/etc/opencode/filebeat-config.yml
{
  "logging": {
    "level": "info",
    "format": "json",
...
生产环境建议配置以下告警规则（详细的 Prometheus 告警规则 JSON 见附录）：
...
对于生产环境，建议使用 Prometheus 的 `predict_linear` 函数做简单的趋势预测：
...
日志中的 `duration_ms` 字段记录了每个步骤的耗时。聚合查询能找到最慢的环节：
...
Token 消耗是 AI 编程助手的主要成本。通过 PromQL 分析消耗分布：
...
1. 收到告警通知，获取 `sessionId`
2. 在日志中搜索 `sessionId` + `level=error`，找到错误事件
3. 从错误事件提取 `traceId`
4. 查询追踪系统获取该 traceId 的完整 Span 列表
5. 分析错误 Span 的 `payload` 获取错误详情（exitCode、stderr）
6. 查看错误 Span 前后的上下文 Span，了解错误发生时的完整场景

这个流程从收到告警到找到根因，通常只需要 2-3 分钟。
