...
> Token 窗口不是无限的，但你的会话可以是。DCP、ACM、Context Guard、Context Manager 四款插件从不同维度解决同一个问题——让 Agent 在有限的上下文窗口内保持高效和准确。
> **适合读者**: 效率开发者 · 架构师 · 高级用户
...
- → [上下文注入](context/context/context-injection-patterns.md)（Context Manager 生成的上下文如何注入）
- ← [自定义 Agent 与 Plugin](custom-agents.md)（Plugin 开发基础）
- ← [性能调优与成本管理](context/performance-tuning.md)（性能优化的另一面）
...
| 策略 | 默认 | 说明 |
|------|------|------|
| **Deduplication** | 启用 | 检测相同工具+相同参数的重复调用，只保留最新输出。在 compress 工具运行时重新计算，只在压缩时影响 Prompt Cache |
...
opencode-context-manager 由 fractalswift 开发，定位与前两者完全不同——它不是优化已有上下文的插件，而是**从代码仓库生成模块化上下文文件**的工具。它更像一个 Skill 而非 Plugin，通过 `/context-update` 命令暴露功能。
