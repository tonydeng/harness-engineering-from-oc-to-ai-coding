...
> MCP（Model **Context（上下文）** Protocol）是 **Agent（智能体）** 与外部系统通信的开放协议。理解它的工作原理、传输方式和安全模型，是扩展 AI 编程能力的关键一步。
> **适合读者**: 后端开发者 · **Skill（技能）** 作者
...
2. **MCP 配置详解** — 三种传输类型的配置方法与选型建议：stdio 适用于本地子进程（低延迟、高安全）、streamable-http 适用于远程服务（灵活部署、跨网络）、websocket ⚠️ 已废弃（2026-07-28 规范移除，不推荐新项目使用）。配置格式围绕 `opencode.json` 中的 `mcpServers` 段展开，包括环境变量管理的最佳实践。
...
- ← [OpenCode 配置深度解析](../03-setup/opencode-config.md)（MCP 在 opencode.json 中的配置位置）
- → [性能调优与成本管理](context/performance-tuning.md)（MCP 调用的性能考量）
...
> **版本说明**（2026-07-28 RC）：MCP 协议即将发布重要变更，影响 Streamable HTTP 传输。主要变化包括：GET 端点已移除（改用 `subscriptions/listen` 替代）、协议级会话（`Mcp-Session-Id`）已移除、SSE 流恢复（`Last-Event-ID`）已移除、WebSocket 传输已移除。新增 `Mcp-Method` 和 `Mcp-Name` 两个必需请求头。如果你正在开发或部署 MCP 服务器，建议提前适配这些变更。
