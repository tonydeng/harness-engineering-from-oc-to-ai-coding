...
| **生态与社区** | Anthropic 生态、**MCP（模型上下文协议）** 协议、社区资源 | [生态参考](./ecosystem.md) |
...
→ 完整命令列表和详细用法见 [Claude Code 命令参考](./commands.md)。
→ 扩展体系详解见 [Claude Code 扩展机制参考](./plugins.md)，涵盖 CLAUDE.md、Skills、MCP、Subagent、Hook、Plugin 六层架构。
→ [Plugin 系统](../../appendix-b/opencode/plugins.md) 详细介绍了 OpenCode 的 Plugin/Skill/MCP 三层扩展架构。
...
> → [Claude Code **Agent（智能体）** 设计与开发指南](./agent-architecture.md) — 自定义 Agent 与 Subagent 的从入门到生产完整教程
> → [Claude **Agent（智能体）** SDK：编程式 Agent 开发](./agent-sdk.md) — 通过 `@anthropic-ai/claude-agent-sdk` 编程式驱动 Agent
> → [OpenCode 内置能力](../../appendix-b/opencode/capabilities.md) — 对应功能的对比参考
...
### 适用读者角色
- 入门开发者 — 需要快速上手 Claude Code，了解其内置命令和工具集
...
### 典型使用场景
- 新项目首次使用 Claude Code 时初始化配置
- 需要快速上手 Claude Code 的内置命令和工具集
- 需要设计和实现自定义 Agent 和 Subagent
- 需要评估 Claude Code 是否适合团队需求
- 需要开发可复用的 Skill 和 MCP 服务器
