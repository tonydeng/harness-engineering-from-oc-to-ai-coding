"""HEDQ 检查模块注册表。
定义 8 个质量维度（D1-D8）及其自动化检测满分权重（原始口径）。

注意：此处 DIM_MAX 是"自动化检测口径"原始满分（非输出百分制）。
...
- 所有输出（Markdown 报告 / JSON / TSV）均缩放为百分制（/100）。
  缩放系数 = 100/58.5（完整模式）或 100/26（快速模式）。
- 未自动化覆盖的子项（D1.3/D2.1/D2.3/D3.2/D3.3/D4.2/D4.3/D4.4/D6.1/D6.2/D6.4）
  共 41.5 分需人工审核。
- D8.3 已实现为信息提示（max_score=0，不参与评分）无 mermaid-cli 时仅做静态代理指标。

如需扩展自动化覆盖范围，实现对应 check 子项后同步更新此处的 DIM_MAX 值。

v3.0 变更（2026-06-30）：
  - 输出统一为百分制（/100），检测逻辑不变
  - runner.py 新增 _scale_factor() 和 _scale_dim() 方法
...
# 各维度自动化检测原始满分（与 check 模块实际产出的子项满分加总一致）。
# 原始总分 = 14+6+6+4+13+2+10+3.5 = 58.5（full）/ 14+2+10 = 26（quick）
# runner.py 在输出层缩放为百分制（/100）。
